...
const validSlugs = ['configuration', 'migration', 'servers', 'security', 'monitoring', 'devops', 'api'];
...
export default async function CloudITSubPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const key = `cloud-it/${slug}`;
  const page = subPagesData[key];
